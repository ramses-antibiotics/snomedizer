...
#'
#' @description A wrapper function for \code{\link{api_concepts}} searching
#' for a term or a list of concepts
#' @param term a string containing search terms
#' @param conceptIds a character vector of one or more SNOMED CT codes
#' to search
#' @param ecl a character expression constraint query (with full inference).
#' Consult the
#' \href{http://snomed.org/ecl}{Expression Constraint Language guide}
#' for more detail.
#' @param activeFilter whether to restrict results to active concepts.
...
#'such as \code{endpoint}, \code{branch} or \code{limit}
#' @return a data frame
#' @section Disclaimer:
#' In order to use SNOMED CT, a licence is required which depends
#' both on the country you are#' based in, and the purpose of your work.
...
#'
#' @description This function compares a vector of \code{concept_ids}
#' identifiers against a target set defined by an ECL expression
#' \code{target_ecl}.
...
#' @param encoding HTTP charset parameter to use (default is \code{"UTF-8"})
#' @return a logical vector with the same order as \code{concept_ids}
#' indicating whether each concept is included in the target set or not.
#' \code{NA} is returned when \code{concept_ids} are not present on the target
#' \code{branch}, or in case of a REST error caused by invalid
#' \code{concept_ids} or \code{target_ecl}.
#' @export
...
#' )
#' concept_included_in(
#'   concept_ids = "39732311000001104",  # Medical product only in UK Edition
#'   target_ecl = "27658006"             # Products containing amoxicillin
#' )
#' concept_included_in(
#'   concept_ids = "233604007",          # Pneumonia
#'   target_ecl = "<<64572001 :          # Disorders with inflammation
#'          116676008 = <<409774005"     # as associated morphology
#' )
#' concept_included_in(
#'   concept_ids = "233604007",          # Pneumonia
#'   target_ecl = "<<64572001 :          # Disorders with inflammation
...
#'
#' @description Wrapper functions of \code{\link{api_concepts}} that fetch
#' ancestorsor descendants of one or several concept identifiers using full
#' SNOMED CT inference.
#'
...
#' # This will trigger a warning using the default limit
#' # set by snomedizer_options_get("limit")
...
#'
#' @description This function is a wrapper of \code{\link{api_descriptions}}
#' that fetches description of one or several concept identifiers.
#' @param conceptIds a character vector of concept identifiers
#' @param encoding HTTP charset parameter to use. Default is \code{"UTF-8"}.
#' @param silent whether to hide progress bar. Default is \code{FALSE}.
#' @param limit a positive integer for the maximum number of results to return.
...
#'
#' @description A wrapper function for the \code{\link{api_refset_members}()}
#' function to query Map Reference Sets, in particular the map to the World
...
#' @param active whether to restrict results to active concepts.
#' Default is \code{TRUE}.
#' @param encoding HTTP charset parameter to use (default is \code{"UTF-8"})
#' @param silent whether to hide progress bar. Default is \code{FALSE}
#' @param limit a positive integer for the maximum number of results to return.
...
#' @examples
#' # find SNOMED CT codes corresponding to ICD-10 code
#' # N39.0 urinary tract infections
...
#' map_icd10 <- concept_map(
#'   concept_ids = c("431308006", "312124009", "53084003")
#' )
