...
test_that("concept_map (batch)", {
  infection_codes <- suppressWarnings(concept_find(ecl = "<40733004",
                                                   limit = 150))
  map_icd10 <- concept_map(concept_ids = infection_codes$conceptId,
                           limit = 500)
  expect_true(all(
  ...
  map_icd10_limit_50 <- concept_map(concept_ids = infection_codes$conceptId,
                                    limit = 50)
  expect_equal(
    map_icd10$refsetId,
